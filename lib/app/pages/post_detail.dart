...
class PostDetail extends StatefulWidget {
  ...
  const PostDetail(this.gelenPost);

  @override
  _PostDetailState createState() => _PostDetailState(gelenPost);
}
...
class _PostDetailState extends State<PostDetail> {
  ...
  _PostDetailState(this.gelenPost);
  ...
  Future<void> _favoriteAdd(gelenID) async {
    final FirebaseFirestore _firestore = FirebaseFirestore.instance;
    UserModel _userModel = Provider.of<UserModel>(context, listen: false);

    List yourItemList = [];
    yourItemList.add(gelenID);
    ...
    if (snapShot == null || !snapShot.exists) {
      await _firestore
          .collection("user_favorite")
          .doc(_userModel.user.userID)
          .set({'favorites':FieldValue.arrayUnion(yourItemList)}).whenComplete(
              () {
            PlatformDuyarliAlertDialog("Success","This ad has been added to favorites.\nYou can see this ad on the favorites page. ","Okey").goster(context);
          } );
    }
    else{
      await _firestore
          .collection("user_favorite")
          .doc(_userModel.user.userID)
          .update({'favorites':FieldValue.arrayUnion(yourItemList)}).whenComplete(
              () {
            PlatformDuyarliAlertDialog("Success","This ad has been added to favorites.\nYou can see this ad on the favorites page. ","Okey").goster(context);
          } );
    }
